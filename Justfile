...
provision-vms:
	@echo "Provisioning QEMU Cloud VMs in tests/e2e..."
...
benchmark target=TARGET:
	...
	just boot-debian > /dev/null 2>&1; \
	echo -n "Waiting for SSH..."; \
	END_BOOT=""; \
	for i in $(seq 1 30); do \
		if ssh -i tests/e2e/e2e_key -p 22221 genesis@localhost -o StrictHostKeyChecking=no -o ConnectTimeout=1 echo "up" > /dev/null 2>&1; then \
			END_BOOT=$(date +%s%3N); \
			echo " Ready."; \
			break; \
		fi; \
	...
	just deploy-debian target={{target}} > /dev/null 2>&1; \
